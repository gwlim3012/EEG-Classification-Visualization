# 뇌파 데이터 분석을 통한 알코올 중독자 분류 및 추상 시각화

## 프로젝트 개요
이 프로젝트는 머신러닝 과목 팀 프로젝트였던 ‘EEG 데이터를 활용한 알코올 중독자 분류’에서 출발한 토이 프로젝트입니다. 당시에는 뇌파 데이터를 FFT(고속 푸리에 변환)를 통해 주파수 영역 특성을 추출하고, 델타·세타·알파·베타·감마 대역별 중앙값을 주요 피처로 사용해 선형 커널 SVM으로 알코올 중독자를 분류했습니다. 그 결과, test data 기준 약 97%의 정확도를 기록했습니다.
...
## 주요 기능
*   **뇌파 시각화**: 뇌파 밴드(델타, 세타, 알파, 베타, 감마)의 파워 데이터를 3D 산점도 형태로 시각화합니다. 각 뇌파 측정치를 별처럼 표현하며, 파워 값의 로그에 비례하여 크기가 달라집니다.
*   **사용자 정의 옵션**:
    *   **포인트 크기**: 포인트의 전체적인 크기를 조절합니다. (기본값: 15, 범위: 1-30)
    *   **Y축 변동성**: 뇌파 밴드 간의 Y축 위치에 무작위 변동성을 추가하여 연속적인 스펙트럼처럼 보이게 합니다. (기본값: 0, 범위: 0-100)
    *   **Z축 변동성**: Z축에 무작위 변동성을 추가합니다. (기본값: 0, 범위: 0-100)
